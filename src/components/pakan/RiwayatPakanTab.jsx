...
const KEPERLUAN_LABELS = {
  ...
  pakan_pagi: "Pakan Pagi",
  pakan_siang: "Pakan Siang",
  pakan_iguana: "Pakan Iguana",
  pakan_baby: "Pakan Baby",
...
const FEED_SOURCE_LABELS = {
  beli_pasar: "Beli di Pasar",
  kebun_sendiri: "Kebun Sendiri",
  panen_azola: "Panen Azola",
  kiriman_supplier: "Kiriman Supplier",
  lainnya: "Lainnya",
};
...
/**
 * Feed transaction history tab.
 * Props:
 *   movements: StockMovement[] (item_type=feedstock)
 *   items: FeedStock[] (for stock reversal on delete)
 *   role: string
 *   onRefresh: () => void
 */
export default function RiwayatPakanTab({ movements, items, role, onRefresh }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <Input placeholder="Cari nama pakan..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-40 h-9" />
        ...
        {(search || typeFilter !== "semua" || dateFrom || dateTo) && (
          <Button variant="ghost" size="sm" className="h-9" onClick={() => { setSearch(""); setTypeFilter("semua"); setDateFrom(""); setDateTo(""); }}>Reset</Button>
        )}
      </div>
      ...
      {filtered.length === 0 ? (
        <Card className="py-12 text-center text-muted-foreground">
          ...
          <p className="text-sm">Belum ada transaksi pakan</p>
        </Card>
      ) : (
        <div className="space-y-2">
          {filtered.slice(0, 100).map((tx) => {
            ...
            const sourceLabel = tx.feed_source ? FEED_SOURCE_LABELS[tx.feed_source] || tx.feed_source : (tx.supplier || null);
            return (
              <Card key={tx.id} className="p-3 flex items-center gap-3 group">
                ...
                {photo && <img src={photo} alt="Bukti" className="w-10 h-10 rounded-lg object-cover border flex-shrink-0" />}
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium truncate">{tx.item_name}</p>
                  <p className="text-xs text-muted-foreground">{safeDateTime(tx.date, tx.created_date)}</p>
                  <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                    {isMasuk ? (
                      sourceLabel && <Badge variant="outline" className="text-[9px] py-0">📦 {sourceLabel}</Badge>
                    ) : (
