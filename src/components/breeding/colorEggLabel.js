...
export async function renderColorLabelHTML(breeding, sizeDef, tortoises = []) {
  ...
  const STATUS_LABEL = { bertelur: "Bertelur", inkubasi: "Inkubasi", menetas: "Menetas", selesai: "Selesai", gagal: "Gagal" };
  const status = STATUS_LABEL[(breeding.status || "").toLowerCase()] || (breeding.status ? breeding.status.charAt(0).toUpperCase() + breeding.status.slice(1) : "—");
  ...
  if (!sizeDef.full) {
    ...
    const candling = `<div style="margin:0 9px 7px;background:${late ? "#FEE2E2" : "#FEF3C7"};border:2px solid ${late ? "#DC2626" : "#B45309"};border-radius:8px;padding:5px 7px;flex-shrink:0">
    ...
    const body = `<div style="flex:1 1 auto;display:flex;flex-direction:row;min-height:0">
      <div style="flex:1 1 auto;display:flex;flex-direction:column;justify-content:space-between;min-width:0">${parentRow}${candling}</div>
      ${qrCol}
    </div>`;
  ...
  const candling = `<div style="margin:9px 14px;background:${late ? "#FEE2E2" : "#FEF3C7"};border:2.5px solid ${late ? "#DC2626" : "#B45309"};border-radius:11px;padding:8px 11px;flex-shrink:0">
  ...
  const suhu = `<div style="font-size:13px;color:#1F2937;padding:7px 14px 5px;flex-shrink:0">Suhu: ${dotted("#1F2937", 90)} °C &nbsp;&nbsp; Kelembapan: ${dotted("#1F2937", 90)} %</div>`;
  ...
  const body = `<div style="flex:1 1 auto;display:flex;flex-direction:row;min-height:0">
    <div style="flex:1 1 auto;display:flex;flex-direction:column;justify-content:space-between;min-width:0">${leftCol}</div>
    ${qrCol}
  </div>`;
