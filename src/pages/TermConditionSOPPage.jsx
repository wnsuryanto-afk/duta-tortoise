/**
 * TermConditionSOPPage — Bentuk 1: tabel master aturan Term & Condition SOP.
 * Owner-only. Mengedit field langsung di entity SOPTask (sumber yang sama
 * yang dibaca checklist keeper), sehingga perubahan langsung berlaku.
 *
 * Kolom inline: Wajib Foto, Boleh Carry-over, Jenis Carry-over,
 * Butuh Bahan Gudang, Poin, Wajib untuk Role. Filter frekuensi & kategori.
 *
 * Catatan: panel ini HANYA mengatur nilai field — belum menjalankan logika
 * carry-over. Alur checklist keeper tidak diubah.
 */
    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-heading font-bold flex items-center gap-2">
          <Settings2 className="w-7 h-7 text-primary" /> Term & Condition SOP
        </h1>
        <p className="text-muted-foreground mt-1">
          Aturan default tiap task SOP — dibaca langsung oleh checklist keeper. Perubahan langsung berlaku pada tugas yang dikerjakan.
        </p>
      </div>
              ...
              {isLoading ? (
                ...
                filtered.map((t) => (
                  <tr key={t.id} className="border-t hover:bg-muted/30">
                    ...
                    <td className="px-3 py-2">
                      <Switch
                        checked={!!t.require_photo}
                        onCheckedChange={(v) => updateField(t, "require_photo", v)}
                      />
                    </td>
                    <td className="px-3 py-2">
                      <Switch
                        checked={!!t.boleh_carryover}
                        onCheckedChange={(v) => updateField(t, "boleh_carryover", v)}
                      />
                    </td>
                    <td className="px-3 py-2">
                      <Select
                        value={t.jenis_carryover || "1hari"}
                        onValueChange={(v) => updateField(t, "jenis_carryover", v)}
                      >
                    ...
                    <td className="px-3 py-2">
                      <Switch
                        checked={!!t.butuh_bahan_gudang}
                        onCheckedChange={(v) => updateField(t, "butuh_bahan_gudang", v)}
                      />
                    </td>
                    <td className="px-3 py-2">
                      <Input
                    ...
                    <td className="px-3 py-2">
                      <Select
                        value={t.wajib_untuk_role || "semua"}
                        onValueChange={(v) => updateField(t, "wajib_untuk_role", v)}
                      >
                        <SelectTrigger className="w-36 h-8 text-xs"><SelectValue /></SelectTrigger>
                        <SelectContent>
      ...
      <p className="text-xs text-muted-foreground">
        Catatan: panel ini hanya mengatur nilai field. Logika carry-over belum dijalankan di sini — alur checklist keeper tidak berubah.
      </p>
    </div>
  );
}