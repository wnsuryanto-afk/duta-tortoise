const fs=require("fs"),path=require("path"),acorn=require("acorn"),esbuild=require("esbuild");
...
for(const f of files){
  ...
  function bd(body){const m=new Map();
    for(const n of body){if(n.type==="VariableDeclaration"&&(n.kind==="const"||n.kind==="let"))
      for(const d of n.declarations){const nm=[];(function ids(x){if(!x)return;
        if(x.type==="Identifier")nm.push(x.name);
      ...
      for(const q of nm) if(!m.has(q)) m.set(q,{start:n.start,line:n.loc.start.line});}}
    return m;}
  function walk(node,chain){
    if(!node||typeof node!=="object")return;
    if(Array.isArray(node)){node.forEach(x=>walk(x,chain));return;}
    let c=chain;
    if(node.type==="BlockStatement"||node.type==="Program")c=chain.concat([bd(node.body)]);
    if(node.type==="Identifier"&&node.start!==undefined){
      for(let i=c.length-1;i>=0;i--){ if(c[i].has(node.name)){
        const d=c[i].get(node.name);
        if(node.start<d.start) out.push(`${f}:${node.loc.start.line} '${node.name}' dipakai sebelum const baris ${d.line}`);
        break;}}}
...
u.filter(x=>/useEffect|guided|sop|incidental|Sidebar|layout/i.test(x)||true).forEach(x=>console.log("  "+x));
